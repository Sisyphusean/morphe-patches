...
// Facebook (com.facebook.katana) and Messenger (com.facebook.orca) share the
// same signing certificate — both issued to Facebook Mobile by Facebook Corporation.
// Extracted from: com.facebook.orca 573.0.0.44.88 (APK Signature Scheme v2).
private const val FACEBOOK_B64 =
...
private val facebookCertSeedPatch = rawResourcePatch(default = false) {
    execute {
        seedCert(FACEBOOK_B64)
    }
}

// No name/description — hidden from the Morphe Manager UI.
// Applied automatically as a dependency of every user-facing Facebook patch.
internal val facebookSignaturePatch = bytecodePatch(default = false) {
    dependsOn(
        facebookCertSeedPatch,
