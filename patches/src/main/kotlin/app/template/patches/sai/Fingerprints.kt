...
// ov6.<init>(Application):V — the OnboardingViewModel constructor that builds the
// page list. It unconditionally adds the paywall slide (vu6.d) as the last page
// regardless of subscription state. We remove that add() call so the paywall page
// is never shown.
//
// Smali: classes4/ov6.smali, method <init>(Application)V (.registers 15)
//   ...
//   sget-object v8, Lvu6;->d:Lvu6;          ← paywall page enum
//   [uu6 construction with resource IDs]
//   invoke-direct/range {v7..v12}, Luu6;-><init>(Lvu6;IIILjava/util/List;)V
//   invoke-virtual {p1, v7}, Lxc5;->add(Object)Z  ← THIS is what we remove
//
// Patch: find the instructionMatches index of the vu6.d sget, then find the
// next xc5.add() call after it and replace it with "const/4 v0, 0x0" (nop-equivalent
// that doesn't change register state — the add() result Z is never used).
//
// Stable anchors:
//   • fieldAccess(SGET_OBJECT, Lvu6;->d) — paywall page enum constant, developer-defined
//   • methodCall(Lxc5;->add) — the page-list add() immediately after
...
// yn5.emit(Object, Lzt1;):Object — the FlowCollector on sr0.d that drives the
// paywall overlay inside MainActivity (shown when returning from onboarding or
// when subscription lapses mid-session).
//
// Smali: classes4/yn5.smali, method emit (.registers 5)
//   p0=Lyn5; p1=Object(Boolean) p2=Lzt1;
...
//     FALSE → read gf8."subscription_prompt_shown" → show paywall overlay
//
// Patch: clearBody() + unconditionally execute the TRUE branch.
//   Registers: v0=COROUTINE_SUSPENDED, v1=null, p0→MainActivity, p1→MainActivity.I
//
// Stable anchors:
