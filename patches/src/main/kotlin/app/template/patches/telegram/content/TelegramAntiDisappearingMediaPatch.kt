...
val telegramAntiDisappearingMediaPatch = bytecodePatch(
    ...
    description = "Keeps view-once photos, videos, voice messages, and round videos viewable indefinitely.",
) {
    ...
    execute {
        ...
        // SecretMediaViewer.closePhoto(ZZ)Z — null out the onClose callback before
        // the original method runs, so the destruction runnable is never queued.
        // TeleVip approach: XposedHelpers.setObjectField(thisObject, "onClose", null)
        // In Morphe: find the iget/iput of the onClose field and replace with const/4 0.
        ...
        Fingerprint(
            ...
            val matchIndices = implementation!!.instructions
                .mapIndexedNotNull { i, insn ->
                    if (insn.opcode == Opcode.IGET_OBJECT) i else null
                }
                .filter { idx ->
                    val insn = getInstruction<com.android.tools.smali.dexlib2.iface.instruction.TwoRegisterInstruction>(idx)
                    insn.toString().contains("onClose")
                }
            matchIndices.reversed().forEach { idx ->
